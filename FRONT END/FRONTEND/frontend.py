...
login = st.Page("pages/login.py")
principal = st.Page("pages/principal.py")
pg = st.navigation([login, principal])
pg.run()


# 1. Configuración visual del tablero
#st.set_page_config(page_title="ViLo Propone", page_icon="logo-mvl.png")
#col1, col2 = st.columns([1, 6])
#with col1:
 #   st.image("logo_pp.jpg", width=100)
#with col2:
   # st.title("Predicción de proyectos nuevos - Presupuesto Participativo")
#st.markdown("Sistema de predicción de viabilidad de nuevos proyectos en Vicente López (modelo Random Forest)")
# 2. Configuración de conexión a la API
# URL_API_BASE = st.sidebar.text_input(
#     "URL de la API", value="http://127.0.0.1:8080"
# )
# API_KEY = st.sidebar.text_input(
#     "API Key (TOKEN_MINISTERIAL)", type="password"
# )
# credenciales = {"x-api-key": API_KEY}
